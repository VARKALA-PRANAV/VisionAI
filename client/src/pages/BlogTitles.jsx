...
const BlogTitles = () => {
  const blogCategoris = ['General','Technology','Business','Health','Lifestyle','Education','Travel','Food']
  ...
  const onSubmitHandler = async (e)=>{
    ...
    try{
      setLoading(true)
      const prompt = `Generate a blog title for the keyword ${input} in the cetegory ${selectedCategory}` 
     ...
     <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div>
          ...
          <input onChange={(e)=>setInput(e.target.value)} value={input} type="text" className='w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4A7AFF]' placeholder='The Feature of Artificial Intelligence is...' required/>
          <p className='mt-5 text-sm font-medium'>Category</p>
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11' >
            {
              blogCategoris.map((item)=>(
